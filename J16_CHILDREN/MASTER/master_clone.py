# this is the benchmark 128 x 128 for the wireless house test
#!/usr/bin/env python3
"""
H.E.R.B.I.E. - J-SERIES (wirelessJ16Cxlog): Jam View
========================================
Same NLSE dynamics as D16C, but optimized for live jamming:

wirelessJ16Cxlog CHANGES (performance layer ONLY):
...
AUDIENT_ID = 1  # Your Audient iD14
LAPTOP_MIC_ID = 2 # Your MacBook Air Mic
SAMPLE_RATE = 44100

np.seterr(divide='ignore', invalid='ignore')

# =============================================================================
# PERSISTENCE - J-SERIES (separate database)
# =============================================================================
J_SERIES = "J16Cxlog"
SAVE_DIR = os.getcwd()
...
class HERBIE:
    def __init__(self):
        ensure_save_dir()
        self.log_file = os.path.join(SAVE_DIR, f"MASTERprosecutor_log_{datetime.now().strftime('%Y%m%d_%H%M%S')}.jsonl")
    ...
    def start(self):
        # 1. Input remains on the Laptop Mic
        self.in_stream = sd.InputStream(
            device=LAPTOP_MIC_ID, 
        ...
        # 2. Output goes to the Audient (Mono Output 1)
        self.out_stream = sd.OutputStream(
            device=AUDIENT_ID,      # Ensure AUDIENT_ID = 1 is at the top of your script
            callback=self._out_cb,
            channels=2,             # Opening the second pipe for your headphones
            samplerate=SAMPLE_RATE,
            blocksize=1024
        )
    ...
    def _out_cb(self, outdata, frames, time, status):
        # 1. Grab the raw simulation value (The 'Force')
        center_val = self.psi[64, 64].real 
        
        # 2. OUTPUT 1 (The Fishman / Left Channel)
        # NO LIMITER. We want the full 0.8 gain to drive the pickup hard.
        # This provides the physical resistance needed to anchor the torus.
        outdata[:, 0] = np.float32(center_val * 0.8)

        # 3. OUTPUT 2 (The Headphones / Right Channel)
        # EAR SAFETY. We scale this way down to 0.15.
        # This is your 'Stethoscope'—you hear the math, but safely.
        outdata[:, 1] = np.float32(center_val * 0.9)
